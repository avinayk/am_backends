              ...
              db.query(query, [user_id, sent_id, "No", date], (err, result) => {
                ...
                db.query(query, [user_id], (err, row) => {
                  ...
                  db.query(
                    ...
                    [row[0].id, sent_id, mesg, date, "/friends"],
                    (err, result) => {
                      if (err) {
                        console.error(
...
exports.getuserChatmessage = async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      ORDER BY  cm.date ASC limit 10 OFFSET ?;
    `;
...
exports.getGalleryGroupforumDetail = async (req, res) => {
  ...
  try {
    ...
    if (post_type === "gallery") {
      ...
      query = `
        SELECT gp.*, 'group' AS pertype,u.username, u.id AS uid, u.profile_type, u.gender, u.profile_image,
          COUNT(gpf.id) AS total_favourites,
          CASE
            WHEN EXISTS (
              SELECT 1 FROM group_post_favourite gpf2 WHERE gpf2.post_id = gp.id
            ) THEN 1
            ELSE 0
          END AS user_favourited
        FROM group_post gp
        ...
        WHERE gp.id = ?
        GROUP BY gp.id, u.username, u.profile_type, u.gender, u.profile_image
      `;
      queryParams = [id];
...
exports.grouplikepostgrouppage = async (req, res) => {
  ...
  const date = moment
    ...
    .format("YYYY-MM-DD HH:mm:ss");
  db.query(
    `SELECT * FROM group_post WHERE user_id = ? AND id = ?`,
    [user_id, id],
    (err, results) => {
      if (results.length > 0) {
                ...
                (insertErr) => {
                  ...
                  db.query(
                    ...
                    (err, row) => {
                      ...
                      logActivity(
                        user_id,
                        `liked a post in the group ${gname}`
                      );
                          ...
                          db.query(
                            ...
                            async (err, senderResult) => {
                              if (err) {
                                return res.status(500).json({
                                  message:
                                    "Error fetching user data for sender",
                                  error: err,
                                });
                              }
